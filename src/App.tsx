...
export default function App() {
  ...
  const stopCamera = useCallback((scrollIntoView: boolean = true) => {
    ...
    // When closing the fullscreen camera in mobile landscape,
    // bring the camera frame back into view smoothly.
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div
          ref={cameraFrameRef}
          className={cn(
            "relative rounded-2xl overflow-hidden bg-black border-2 border-neutral-800 camera-frame",
            isCameraActive && "camera-frame-active"
          )}
        >
          {/* Video area */}
          <div className="relative aspect-[16/9] bg-neutral-900 video-area">
            {!isCameraActive ? (
              ...
              <>
                ...
                {showOverlay && (
                  <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                    <img
                      ...
                      className={cn(
                        "silhouette-img object-contain transition-all duration-300",
                        isAligned ? 'opacity-55 scale-100' : 'opacity-30 scale-[0.98]'
                      )}
                      style={{ mixBlendMode: 'screen', width: '78%', maxWidth: '600px' }}
                    />
                  </div>
                )}

                {/* Alignment Status */}
                <div className="alignment-status absolute top-4 right-4">
                  <div
                    className={cn(
                ...
                {/* Instructions / Angle card */}
                <div className="angle-card absolute bottom-4 left-4 right-4 md:right-auto md:max-w-xs">
          ...
          {/* Camera Controls */}
          {isCameraActive && (
            <div className="camera-controls bg-neutral-900 p-4 border-t border-neutral-800">
              <div className="controls-row flex items-center justify-center gap-6">
                <button
                  onClick={() => stopCamera()}
                  className="reset-btn p-3 rounded-full bg-neutral-700 hover:bg-neutral-600 transition-colors"
                ...
                <button
                  ...
                  disabled={!isAligned}
                  className={cn(
                    "capture-btn w-20 h-20 rounded-full flex items-center justify-center transition-all border-4",
                    isAligned
                ...
                {/* Prev / Next angle buttons - shown only in mobile landscape via CSS */}
                <button
                  onClick={() => goToAngle(currentAngleIndex - 1)}
                  disabled={currentAngleIndex === 0}
                  className="angle-nav-btn prev-angle-btn p-3 rounded-full bg-neutral-700 hover:bg-neutral-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                ...
                <button
                  onClick={() => goToAngle(currentAngleIndex + 1)}
                  disabled={currentAngleIndex === CAR_ANGLES.length - 1}
                  className="angle-nav-btn next-angle-btn p-3 rounded-full bg-neutral-700 hover:bg-neutral-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
